...
public class DiskLruCache {
	...
	private final static LinkedHashMap<String, String> keyMaps = new LinkedHashMap<String, String>();
...
////						flushCache();
//					}else{
//						if (writeBitmapToFile(data, existingFile)) {
//							put(key, existingFile);
//							flushCache();
//						}
//					}
...
//				}
	...
	private void flushCache2() {
		Entry<String, String> eldestEntry;
		File eldestFile;
		long eldestFileSize;
		while(mLinkedHashMap.size() > 30){
...
//		clearDiskCache();
	...
	public static void clearDiskCache(File cacheFile){
		File[] listFiles = cacheFile.listFiles();
		if(listFiles.length > 200){
			for(File file : listFiles){
				if(keyMaps.get(file.getName()) == null){
					file.delete();
				}
			}
		}
	...
	public static int getCacheSize(File cacheFile){
		File[] listFiles = cacheFile.listFiles();
		if(listFiles != null){
...
//		while (count < MAX_REMOVALS && (cacheSize > maxCacheItemSize || cacheByteSize > maxCacheByteSize)) {
		while (count < MAX_REMOVALS && (cacheSize > maxCacheItemSize)) {
			eldestEntry = mLinkedHashMap.entrySet().iterator().next();
			eldestFile = new File(eldestEntry.getValue());
			eldestFileSize = eldestFile.length();
			mLinkedHashMap.remove(eldestEntry.getKey());
...
//		}
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		if(android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {
			cachePath = android.os.Environment.getExternalStorageDirectory() + File.separator + "playtv";
		}else{
	...
	public static String createFilePath(File cacheDir, String key) {
		try {
			...
			// this example
			String basePath = cacheDir.getAbsolutePath() + File.separator + CACHE_FILENAME_PREFIX + URLEncoder.encode(key.replace("*", ""), "UTF-8");
//			int beginIndex = basePath.indexOf("http");
//			int endIndex = basePath.indexOf("EPG");
//			String removeStr = basePath.substring(beginIndex, endIndex);
//			String path = basePath.replace(removeStr, "");
			return basePath;
